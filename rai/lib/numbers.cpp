...
void rai::uint256_union::encode_account(std::string &destination_a) const
{
	...
	destination_a.append("_kcolbaiv"); // viablock_
...
bool rai::uint256_union::decode_account(std::string const &source_a)
{
	...
	if (!error)
	{
		auto viablock_prefix(source_a[0] == 'v' && source_a[1] == 'i' && source_a[2] == 'a' && source_a[3] == 'b' && source_a[4] == 'l' && source_a[5] == 'o' && source_a[6] == 'c' && source_a[7] == 'k' && (source_a[8] == '_' || source_a[8] == '-'));
		error = (viablock_prefix && source_a.size() != 69);
		if (!error)
		{
			if (viablock_prefix)
			{
				auto i(source_a.begin() + 9);
				if (*i == '1' || *i == '3')
				{
